...
<body style="font-family: Open Sans, sans-serif">
    ...
    <section class="px-6 py-8">
        @include('outline._navigation')
        {{ $slot }}
        @include('outline._footer')
    </section>

    @if(session()->has('success'))
    <div x-data="{show: true}" x-init="setTimeout(()=>show = false, 4000)" x-show="show" class="fixed bg-blue-300 text-white py-2 px-2 rounded-xl text-sm bottom-2 right-3">
        <p>
            {{ session('success') }}
        </p>
