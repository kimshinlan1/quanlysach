            ...
            success: function success(response) {
              if (response.length < 0) {
                ...
                for (var i = 0; i < response.length; i++) {
                  ...
                  tr.append('<td>' + response[i].category.tendanhmuc + '</td>');
                  ...
                  let link = $("<a></a>").addClass("btn btn-primary").attr("href", "{{ route('book.edit', $book->id) }}").text("Sửa");
                  // create the csrf input field
                  let csrfInput = $("<input>").attr({
                    type: "hidden",
                    name: "_token",
                    value: "{{ csrf_token() }}"
                  });
                  // create the delete button element
                  let deleteButton = $("<button></button>").addClass("btn btn-danger btn-delete").attr({
                    type: "submit",
